...
// New 创建一个新的异步执行对象
func New() Async {
	return Async{tasks: make(map[interface{}]asyncRun)}
}
...
// params 任务执行函数所需要的参数
func (a *Async) Add(name interface{}, handler interface{}, params ...interface{}) bool {
...
func (a *Async) Run() (chan map[interface{}][]interface{}, bool) {
	if a.count < 1 {
		return nil, false
	}
	result := make(chan map[interface{}][]interface{})
	chans := make(chan map[string]interface{}, a.count)

	a.wg.Add(a.count)
	go func(result chan map[interface{}][]interface{}, chans chan map[string]interface{}) {
		rs := make(map[interface{}][]interface{})
		defer func(rs map[interface{}][]interface{}) {
			result <- rs
		}(rs)
		for {
			if a.count < 1 {
			...
			case res := <-chans:
				a.count--
				rs[res["name"]] = res["result"].([]interface{})
			}
		}
	}(result, chans)

	for k, v := range a.tasks {
		go func(name interface{}, chans chan map[string]interface{}, async asyncRun) {
			defer a.wg.Done()
			result := make([]interface{}, 0)
			defer func(name interface{}, chans chan map[string]interface{}) {
...
// Clean 清空任务队列.
func (a *Async) Clean() {
	a.tasks = make(map[interface{}]asyncRun)
}
